...
day9.calculateLength = function (input, multiplier) {
    ...
    while (position < input.length) {
        if (input[position] === '(') {
            ...
            length += insideLength;                    
            if(previousMarker !== undefined && previousMarker.content === marker.content){
                position += input.substring(position).indexOf(marker.content) + 1 + marker.content.length + substring.length;
            } 
            else{
